...
class ErrorRecoveryService
    ...
    constructor(
            ...
            flow {
                ...
                repeat(maxRetries) { attempt ->
                    try {
                        ...
                        debugLogManager.log(
                            "ERROR_RECOVERY",
                            "$operationName - Attempt ${attempt + 1} failed: ${e.message}",
                        )
                ...
                try {
                    ...
                    if (e.message?.contains("rate limit", ignoreCase = true) == true ||
                        ...
                        try {
                            ...
                            debugLogManager.log(
                                "ERROR_RECOVERY",
                                "Retry after rate limit failed: ${retryException.message}",
                            )
                ...
                repeat(maxRetries) { attempt ->
                    try {
                        ...
                        if (isNetworkError(e)) {
                            debugLogManager.log(
                                "ERROR_RECOVERY",
                                "Network error on attempt ${attempt + 1}: ${e.message}",
                            )
        ...
        class CircuitBreaker(
            ...
            suspend fun <T> execute(operation: suspend () -> T): Flow<Result<T>> =
                flow {
                    when (state) {
                        CircuitState.OPEN -> {
