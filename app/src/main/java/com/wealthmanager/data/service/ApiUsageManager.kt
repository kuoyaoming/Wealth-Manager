...
class ApiUsageManager
    ...
    constructor(
        ...
        suspend fun canMakeRequest(): Boolean {
            ...
            if (requestsToday.get() >= getDailyLimit()) {
                debugLogManager.logWarning(
                    "Daily API limit reached: ${requestsToday.get()}/${getDailyLimit()}",
                    "API_USAGE",
                )
                return false
            }
            ...
            if (requestsThisMinute.get() >= getMinuteLimit()) {
                debugLogManager.logWarning(
                    "Minute API limit reached: ${requestsThisMinute.get()}/${getMinuteLimit()}",
                    "API_USAGE",
                )
                return false
            }
        ...
        fun recordRequest() {
            ...
            debugLogManager.log(
                "API_USAGE",
                "API request recorded. This minute: ${requestsThisMinute.get()}/${getMinuteLimit()}, Today: ${requestsToday.get()}/${getDailyLimit()}",
            )
        }
