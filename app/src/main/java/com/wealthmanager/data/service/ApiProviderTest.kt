        ...
        suspend fun testStockQuote(symbol: String = "AAPL"): Boolean {
            return try {
                ...
                if (result.isSuccess) {
                    ...
                    debugLogManager.log(
                        "API_TEST",
                        "Stock quote test passed: ${quoteData.symbol} = ${quoteData.price} (Provider: ${quoteData.provider})",
                    )
                    true
                } else {
                ...
                apiProviderService.searchStocks(query, "US").collect { result ->
                    when (result) {
                        is com.wealthmanager.data.model.SearchResult.Success -> {
                            debugLogManager.log(
                                "API_TEST",
                                "Stock search test passed: Found ${result.results.size} results",
                            )
                            success = true
                        }
        ...
        suspend fun testExchangeRate(): Boolean {
            return try {
                ...
                if (result.isSuccess) {
                    ...
                    debugLogManager.log(
                        "API_TEST",
                        "Exchange rate test passed: ${rateData.fromCurrency}/${rateData.toCurrency} = ${rateData.rate} (Provider: ${rateData.provider})",
                    )
                    true
                } else {
