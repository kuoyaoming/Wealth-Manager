        ...
        suspend fun updateStockPrices() {
            try {
                ...
                for (stock in stockAssets) {
                    try {
                        val result =
                            apiRetryManager.executeWithFallback(
                                operation = {
                                    ...
                                    if (quoteResult.isSuccess) {
                                        ...
                                        throw Exception(
                                            "API Provider failed: ${quoteResult.exceptionOrNull()?.message}",
                                        )
                                    }
                                },
                                fallbackOperation = {
                                    debugLogManager.logWarning(
                                        "MARKET_DATA",
                                        "API Provider failed for ${stock.symbol}, using cached data",
                                    )
                        ...
                        if (result.isSuccess) {
                            ...
                            debugLogManager.log(
                                "MARKET_DATA",
                                "Failed to update ${stock.symbol}, keeping existing price",
                            )
                        }
                    } catch (e: Exception) {
            ...
            return object {
                ...
                val lastRefreshed =
                    java.text.SimpleDateFormat(
                        "yyyy-MM-dd HH:mm:ss",
                    ).format(java.util.Date(cachedRate.lastUpdated))
            }
        }
        ...
        suspend fun updateExchangeRates() {
            try {
                ...
                val result =
                    apiRetryManager.executeWithFallback(
                        ...
                        fallbackOperation = {
                            debugLogManager.logWarning(
                                "MARKET_DATA",
                                "API Provider failed for exchange rate, using cached data",
                            )
                ...
                if (cachedRate != null) {
                    debugLogManager.logWarning(
                        "MARKET_DATA",
                        "Using cached exchange rate as fallback: ${cachedRate.rate}",
                    )
                }
            }
        }
        ...
        suspend fun searchStocks(
            ...
            flow {
                try {
                    ...
                    debugLogManager.logMarketData(
                        "ERROR",
                        "Search failed for '$query' in market '$market': ${e.message}",
                    )
