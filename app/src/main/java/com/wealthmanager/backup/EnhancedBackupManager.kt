...
class EnhancedBackupManager @Inject constructor(
    ...
    suspend fun getBackupStatus(): BackupStatus {
        ...
        val storedKeyTypes = if (isGooglePasswordManagerAvailable) {
            credentialManagerService.listStoredKeyTypes()
        } else {
            emptyList()
        }
        
        val strategy = determineBackupStrategy(
            isLocalBackupEnabled,
            isGooglePasswordManagerAvailable,
            isGoogleAccountSignedIn
        )
        
        debugLogManager.log("ENHANCED_BACKUP", "Backup status: $strategy, Local: $isLocalBackupEnabled, Google: $isGooglePasswordManagerAvailable")
