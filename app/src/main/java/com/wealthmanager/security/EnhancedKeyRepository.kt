 ...
 */
@Singleton
class EnhancedKeyRepository @Inject constructor(
    ...
    suspend fun getSmartFinnhubKey(): String? {
        return try {
            ...
            debugLogManager.log("ENHANCED_KEY_REPO", "No Finnhub key found in local storage or Google Password Manager")
            null
        } catch (e: Exception) {
    ...
    suspend fun getSmartExchangeKey(): String? {
        return try {
            ...
            debugLogManager.log("ENHANCED_KEY_REPO", "No Exchange key found in local storage or Google Password Manager")
            null
        } catch (e: Exception) {
    ...
    suspend fun setSmartFinnhubKey(
        ...
        if (validation.isValid) {
            try {
                ...
                if (syncToGoogle) {
                    val googleResult = credentialManagerService.saveApiKeyToGooglePasswordManager(
                        "finnhub", 
                        trimmedValue
                    )
                    if (googleResult.isSuccess) {
                        ...
                        debugLogManager.logError("ENHANCED_KEY_REPO", "Failed to sync Finnhub key to Google Password Manager")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (syncToGoogle) {
                    ...
                    if (googleResult.isSuccess) {
                        ...
                        debugLogManager.logError("ENHANCED_KEY_REPO", "Failed to sync Exchange key to Google Password Manager")
                    }
                }
            } catch (e: Exception) {
    ...
    suspend fun clearSmartFinnhubKey() {
        ...
        debugLogManager.log("ENHANCED_KEY_REPO", "Cleared Finnhub key from local storage (Google Password Manager requires manual deletion)")
    }
    ...
    suspend fun clearSmartExchangeKey() {
        ...
        // Note: Google Password Manager doesn't provide a direct way to delete credentials
        // The user would need to manually delete them from their Google account
        debugLogManager.log("ENHANCED_KEY_REPO", "Cleared Exchange key from local storage (Google Password Manager requires manual deletion)")
    }
