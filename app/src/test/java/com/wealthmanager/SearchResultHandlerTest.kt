...
class SearchResultHandlerTest {
    ...
    fun `test isErrorState with empty success results`() {
        ...
        assertTrue("Empty success results should be error state", SearchResultHandler.isErrorState(searchResult))
    }

    @Test
    fun `test isErrorState with non-empty success results`() {
        val searchResult = SearchResult.Success(listOf(
            StockSearchItem("AAPL", "Apple Inc.", "Apple Inc.", "NASDAQ", "OPEN")
        ))
        assertFalse("Non-empty success results should not be error state", SearchResultHandler.isErrorState(searchResult))
    }

    @Test
    fun `test isApiLimitError with API limit reached`() {
        val searchResult = SearchResult.NoResults(NoResultsReason.API_LIMIT_REACHED)
        assertTrue("API limit reached should be API limit error", SearchResultHandler.isApiLimitError(searchResult))
    }

    @Test
    fun `test isApiLimitError with API limit error type`() {
        val searchResult = SearchResult.Error(SearchErrorType.API_LIMIT)
        assertTrue("API limit error type should be API limit error", SearchResultHandler.isApiLimitError(searchResult))
    }
    ...
    fun `test isStockNotFoundError with stock not found reason`() {
        ...
        assertTrue("Stock not found reason should be stock not found error", SearchResultHandler.isStockNotFoundError(searchResult))
    }
